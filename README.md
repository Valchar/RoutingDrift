...
Mixture-of-Experts (MoE) models like OLMoE and Mixtral route each token through a small subset of specialized "expert" sub-networks instead of running every token through the same dense network. This makes them parameter-efficient at inference time, but it also makes them surprisingly tricky to optimize. The routing decision is dynamic and data-dependent, which breaks a surprising number of tools we normally reach for: `torch.compile` trips over the routing logic and can't fuse key ops, quantizing the weights shifts the routing distribution in ways that may or may not hurt accuracy, and custom kernels can interact badly with quantized weight formats.

This project investigates all three of those problems together. We picked two real MoE models — **OLMoE-1B-7B** (64 experts, top-8 routing, 16 layers) and **Mixtral-8x7B-GPTQ** (8 experts, top-2 routing) — and ran three independent but complementary sub-studies on an A100 GPU on the Zaratan HPC cluster at UMD.
...
We wrote hand-tuned Triton kernels for two ops that appear in every layer of both models: **RMSNorm** and the routing **Softmax**. The goal was to fuse the two-pass variance + normalize computation for RMSNorm into a single kernel pass, and do the same for the row-wise softmax over expert gate logits.

In isolation, these kernels are genuinely fast:
...
But here's the catch — Amdahl's Law. RMSNorm occupies only **1.91% of OLMoE's forward pass time**, and Softmax occupies essentially nothing. So even a 7.3x isolated speedup gives a predicted E2E ceiling of just **1.015x**. The measured E2E confirmed this: OLMoE with Triton kernels runs at **0.985x** the baseline at seq=512 batch=4 — the kernel's launch overhead doesn't amortize at small batch sizes.

On Mixtral-8x7B-GPTQ the result was much worse: **0.037x measured speedup** — effectively a 27x regression. The GPTQ-quantized weights use a packed INT4 memory layout that causes extreme memory-access serialization when our Triton RMSNorm kernel runs alongside it. This is a kernel–quantization format mismatch, not a bug in the kernel itself.

**Bottom line:** Triton kernels are correct and memory-efficient. The E2E ceiling is set by Amdahl, not kernel quality. Never apply FP16-tuned Triton kernels to GPTQ checkpoints.

---

### 2. Routing Drift Under Quantization (`quantization/`)

The central question: when you quantize a MoE model to INT8 or INT4, does the routing change? If tokens get sent to completely different experts after quantization, the model's specialized knowledge is effectively scrambled — even if the numerical outputs look close.

We hook the gate layer in OLMoE-1B-7B, run the same prompts through FP16, INT8, and INT4, and compare expert selections using four metrics:
...
The routing is remarkably stable. At INT8, 95.5% of tokens still select the same experts as FP16. At INT4, still 93.3%. The reason: quantization shifts gate logit values slightly, but the top-k selection is dominated by large margin differences between experts — small perturbations rarely flip the ranking. Degradation is also monotonic across all four metrics: FP16 > INT8 > INT4.

We also ran a per-layer breakdown across all 16 layers, capturing which layers drift most under quantization — useful input for future mixed-precision schemes that could selectively protect the most routing-sensitive layers.

For an accuracy reference point we ran `lm-eval` on the FP16 model:

| Benchmark  | Score  | Notes                                          |
|------------|--------|------------------------------------------------|
| MMLU       | 52.8%  | Slightly above chance, expected for 1B active params |
| HellaSwag  | 78.3%  | (acc_norm) Solid commonsense performance       |
| GSM8K      | 8.1%   | Matches ~8% in the OLMoE paper                 |

Closing the loop with INT8/INT4 accuracy evals to verify that low routing drift actually preserves downstream accuracy is the primary next step for this sub-study.

---

### 3. Why `torch.compile` Struggles with MoE (`Compiler/`)

`torch.compile` traces PyTorch code into a computation graph and fuses ops via TorchInductor. It works brilliantly on dense models. MoE routing breaks it because the routing logic is inherently data-dependent — `torch.compile` can't trace through dynamic branches or dynamic shapes, so it stops and resumes in eager mode at those points.

Both OLMoE and Mixtral produce **exactly 1 graph break**, both in the MoE routing layer. The compiled subgraphs cover Attention and FFN ops — which trace cleanly — but the routing kernel itself always executes in eager mode. This means only **50% of the graph** is ever compiled for either model.
...
Two things stand out. First, `default` mode is actually *slower* than eager for both models — the compilation overhead outweighs the benefit when half the graph runs in eager anyway. Second, the best available gains are modest: 3.2% for OLMoE (`reduce-overhead`) and 0.5% for Mixtral (`max-autotune`). The 1 graph break in the routing layer is a hard structural ceiling.

`torch.compile(dynamic=True)` is the most promising path to removing the break entirely (untested in this study).
...
│   ├── benchmark.py                 # E2E latency sweep (seq_len × batch_size)
...
│       ├── routing_drift_summary.csv    # Per-precision drift metrics (4 metrics × 3 precisions)
...
└── report/                          # Cross-study aggregation
    ...
    ├── permutation_report.py        # Full permutation grid (precision × kernel × compile)
...
### Sub-study 1 — Kernel Optimization *(all steps require GPU)*
...
Sweeps all combinations of `seq_len ∈ {128, 512, 1024}` and `batch_size ∈ {1, 4}`. Measures latency p50/p90/p99, throughput (tokens/sec), peak VRAM, and speedup. Outputs `benchmark_<model>.csv`.
...
Benchmarks RMSNorm and Softmax in isolation across hidden sizes, then profiles a full model forward pass to measure what fraction of runtime each op occupies. Outputs `profile_rmsnorm_isolated.csv`, `profile_softmax_isolated.csv`, `profile_model_ops_baseline.csv`, `profile_model_ops_kernel.csv`, and `profile_amdahl.csv`.
...
1. **Triton kernels hit large isolated speedups but minimal E2E gain on OLMoE** — RMSNorm achieves 7.3x in microbenchmark but occupies only 1.91% of total runtime, giving an Amdahl ceiling of 1.015x. Measured E2E is 0.985x at seq=512.

2. **Triton kernels catastrophically regress on Mixtral-GPTQ** — 0.037x measured speedup (27x slower) due to packed INT4 memory layout incompatibility. The kernel is correct; the format is incompatible.

3. **Quantization barely moves the routing distribution** — INT8 achieves 95.5% routing similarity to FP16; INT4 achieves 93.3%. Expert selection is robust because ranking margins between experts are large enough that small quantization perturbations rarely flip the top-k.
...
5. **INT8 quantization is the most deployable optimization overall** — lowest routing drift (4.6%), no compilation instability, works on both model families. The ~7% drift at INT4 is still well within the stable routing regime.
